...
qint32 ClPresenterL1ex::open()
{
	...
	if(m_lProductId == GETPRODUCTID(PRODUCT_L1EX)){				//L-1EXの場合
		m_pUsbDevice = new ClIoDevice(&GUID_L1EX);
	}else if(m_lProductId == GETPRODUCTID(PRODUCT_P10)){		//P10の場合
		m_pUsbDevice = new ClIoDevice(&GUID_P10);
	}else{														//失敗
