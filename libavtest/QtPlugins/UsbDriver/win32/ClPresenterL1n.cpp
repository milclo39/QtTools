...
qint32 ClPresenterL1n::open()
{
	...
	if(m_lProductId == GETPRODUCTID(PRODUCT_L1N))				//L-1Nの場合
	{
		m_pUsbDevice = new ClIoDevice(&GUID_L1N);				//L-1Nの場合
	}
	else if(m_lProductId == GETPRODUCTID(PRODUCT_P10S))			//P10Sの場合
	{
		m_pUsbDevice = new ClIoDevice(&GUID_P10S);				//P10Sの場合
	}
	else if(m_lProductId == GETPRODUCTID(PRODUCT_P30S))			//P30Sの場合
	{
		m_pUsbDevice = new ClIoDevice(&GUID_P30S);				//P30Sの場合
	}
	else if(m_lProductId == GETPRODUCTID(PRODUCT_FF1))			//FF-1の場合
	{
		m_pUsbDevice = new ClIoDevice(&GUID_FF1);				//FF-1の場合
	}
	else
	{
	...
	if(m_pUsbDevice->OpenWriteEP() != TRUE)						//デバイスファイルオープン L-1nでは使用しない
	{
		return IM_ERROR_NO_DEVICE;
	}
	if(m_pUsbDevice->OpenWriteEP2() != TRUE)					//デバイスファイルオープン L-1nでは使用しない
	{
		return IM_ERROR_NO_DEVICE;
	}
