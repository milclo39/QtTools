...
void  get_license_data(mc_license_command_id id, void *pData)
{
    uint32_t rsize = 0;
    ...
    if (id == GET_SIZE_IN_BYTE)
        *(uint32_t*)pData = rsize;
}
...
void * MC_EXPORT_API get_rc(const char* name)
{
    ...
    else if (!strcmp(name, "get_license_data"))
        return (void*) get_license_data;
