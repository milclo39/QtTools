...
{
    //fourcc,     bit_depth, chroma_format, bits_per_pixel, pixels_per_macropixel
    // YUV
...
*/
const static uint8_t* get_name(uint32_t fourcc)
{
    for (int i = 0; i < g_ColorSpaceBaseInfoMapSize; i++)
...
*/
static uint32_t get_fourcc(const uint8_t* name)
{
    for (int i = 0; i < g_ColorSpaceBaseInfoMapSize; i++)
    {
        bool equal_names = !strcmp((const char*)name, (const char*)g_ColorSpaceBaseInfoMap[i].name);
        bool equal_alternative_names = strcmp((const char*)"", (const char*)g_ColorSpaceBaseInfoMap[i].alternative_name) &&
...
/** @brief Call this function to get type of color space (RGB, YUV, XYZ) by fourcc.
	 ...
	 */
static int32_t get_cs_type(uint32_t fourcc)
{
    for(int i = 0; i < g_ColorSpaceBaseInfoMapSize; i++)
        if(g_ColorSpaceBaseInfoMap[i].fourcc == fourcc)
...
/** @brief Call this function to get bit depth by fourcc.
	 ...
	 */
static int32_t get_bit_depth(uint32_t fourcc)
{
    for(int i = 0; i < g_ColorSpaceBaseInfoMapSize; i++)
        if(g_ColorSpaceBaseInfoMap[i].fourcc == fourcc)
...
/** @brief Call this function to get chroma format by fourcc.
	 ...
	 */
static int32_t get_chroma_format(uint32_t fourcc)
{
    for(int i = 0; i < g_ColorSpaceBaseInfoMapSize; i++)
        if(g_ColorSpaceBaseInfoMap[i].fourcc == fourcc)
...
/** @brief Call this function to count of bits per pixel by fourcc.
     ...
     *         0 if FourCC format is unsupported
	 */
static uint32_t get_bits_per_pixel(int32_t fourcc)
{
    for(int i = 0; i < g_ColorSpaceBaseInfoMapSize; i++)
        if(g_ColorSpaceBaseInfoMap[i].fourcc == fourcc)
...
/** @brief Call this function to get number of pixels per macropixel.
     ...
     *         0 if FourCC format is unsupported
	 */
static uint32_t get_pixels_per_macropixel(int32_t fourcc)
{
    for(int i = 0; i < g_ColorSpaceBaseInfoMapSize; i++)
        if(g_ColorSpaceBaseInfoMap[i].fourcc == fourcc)
...
/** @brief Call this function to check color space frame parameters.
     ...
     *         1 if not
	 */
static int32_t check_frame_colorspace_param(uint32_t width, uint32_t height, uint32_t fourcc)
{
    uint32_t pixels;
    ...
    int32_t chroma_format = get_chroma_format(fourcc);
...
/** @brief Call this function to get color space frame information.
     ...
     *         1 if not
	 */
static int32_t get_frame_colorspace_info(frame_colorspace_info_tt *cs_info, uint32_t width, uint32_t height, uint32_t fourcc, int32_t stride)
{
    if(!cs_info)
        return 1;
...
/** @brief Call this function to fill @ref frame_tt structure from @ref frame_colorspace_info_tt.
     ...
     *         1 if not
	 */
static int32_t fill_frame_from_colorspace_info(const frame_colorspace_info_tt *cs_info, uint8_t *buffer, frame_tt *frame_tt_out)
{
    ...
    for(uint32_t i = 0; i < cs_info->planes; i++)
    {
...
/** @brief Call this function to get offsets to the position for each plane in bytes.
     ...
     *         1 if not
	 */
static int32_t get_plane_offsets_in_bytes(uint32_t *offset, uint32_t x, uint32_t y, uint32_t fourcc, int32_t stride)
{
    if(!offset)
        return 1;
...
/** @brief Call this function to flip color space frame.
     ...
     *         1 if not
	 */
static int32_t flip_colorspace(frame_colorspace_info_tt *cs_info)
{
    if(!cs_info)
        return 1;

    for(int8_t i = 0; i < 4 && (cs_info->stride[i] != 0); i++)
    {
