							...
							AVPixelFormat eFormat)
{
	...
	//出力フレーム準備
	int iDestBufferSize = avpicture_get_size(eFormat, pFrameDest->width, pFrameDest->height);
	uint8_t *ucBufferDest = (uint8_t*)av_malloc(iDestBufferSize);
	if(0 >= avpicture_fill((AVPicture*)pFrameDest, ucBufferDest, eFormat,
				   pFrameDest->width, pFrameDest->height))
	{
		return false;
	}
	...
	SwsContext *pSws = sws_getContext(pFrameSource->width, pFrameSource->height,
							  (AVPixelFormat)pFrameSource->format, pFrameDest->width, pFrameDest->height,
